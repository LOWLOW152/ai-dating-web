...
// 第二层AI评分提示词 - 相似度打分（含详细维度评分规则）
const LEVEL2_PROMPT = `你是交友匹配助手。请比较两个人的以下5个维度，给出相似度评分（0-100分）。

【评分维度及详细规则】

1. 兴趣爱好（0-100分）
   - 先判断兴趣类型：
     * 运动类：跑步、健身、游泳、球类、户外、爬山等
     * 文艺类：阅读、写作、绘画、音乐、看电影、追剧等  
     * 社交类：聚会、桌游、剧本杀、K歌、逛街等
     * 静享类： cooking、喝茶、冥想、养花草、手工等
     * 游戏类：手游、端游、主机游戏、电竞等
   - 评分标准：
     * 90-100分：类型相同（如都是运动类），且有2+个具体项目重叠
     * 70-89分：类型相同，但具体项目不同（如都喜欢运动，一个跑步一个游泳）
     * 50-69分：一动一静（如运动+阅读），或类型差异但能互相理解
     * 30-49分：类型完全不同，几乎没有交集
     * 0-29分：一方有大量爱好，另一方几乎没有
