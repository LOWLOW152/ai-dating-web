...
function buildPrompt(
  ...
  const retryMarker = isRetry 
    ? `【重要】这是用户要求重新回答本题。用户可能觉得刚才的对话有问题（比如AI卡死、回答不理想等）。

【处理方式】
1. 你已经说了"哎呀，刚才走神了，我们重新聊一下这个话题吧~"（这句话会显示给用户）
2. 现在自然地重新引入本题的问题，就像重新开始这个话题一样
3. 可以忽略之前关于本题的不理想对话，重新提问
4. 如果用户之前已经回答过一些内容，你可以参考，但不要直接重复之前的追问

【当前题目】${question.question_text}
...
`;
  ...
  // 无论是否是最后一轮，都使用支持静默结束的格式说明
  // 旧的 cfg.data_format_template 要求必须有自然语言回复，与静默结束冲突
  const formatTemplate = isLastRound ? `
【返回格式要求 - 强制静默模式】
由于是最后一轮追问，本次回复**只返回DATA，不返回任何对话内容**。

格式：
第一部分：（留空，什么都不要写，不要回复"收到""好的"等任何内容）

---DATA---

第二部分：当前题提取的数据（JSON格式）

重要：第一部分必须完全留空，直接静默进入下一题。
` : `
【返回格式要求】
你的回复必须包含两部分，用 ---DATA--- 分隔：

第一部分：对用户的自然语言回复（追问），**但如果数据已完整则留空**
...
【重要判断】
- 如果用户回答不完整需要追问 → 第一部分写追问内容
- 如果用户回答完整无需追问 → 第一部分留空（静默结束）
`;
