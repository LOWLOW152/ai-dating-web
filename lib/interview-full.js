...
export function extractAnswer(message, questionKey, questionType) {
  ...
  if (questionKey === 'gender') {
    // 性别只有两种，简单明确
    if (['男', '男生', '女的', '女', '男生', '女生'].includes(trimmed)) {
      const value = trimmed.includes('男') ? '男' : '女';
      return { value, originalValue: trimmed, confidence: 0.98, needsConfirm: false };
    }
  }
  ...
  if (questionKey === 'birth_year') {
    ...
    if (yearMatch) {
      ...
      if (year >= 1970 && year <= 2010) {
        ...
        confidence = 0.7; // 年份不太合理，但不过度质疑
      }
    } else {
      confidence = 0.8; // 没匹配到标准年份格式，但仍给高置信度
    }
  }
  ...
  return {
    ...
    confidence,
    needsConfirm: confidence < 0.7 // 只有置信度低于0.7才需要确认
  };
}
