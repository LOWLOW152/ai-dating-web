...
registerAlgorithm('keyword_blocker',
  function keywordBlocker(answerA, answerB, config = {}) {
    ...
    const parseSet = (ans) => {
      if (!ans) return [];
      return String(ans).split(RegExp(`[${separator}，、]`)).map(s => s.trim()).filter(Boolean);
    };
    ...
    if (union.length === 0) {
      return { score: 70, details: { reason: '无数据，默认分数' } };
    }
    
    // 共同红线越多，说明价值观越一致，分数越高
    const similarity = Math.round((common.length / union.length) * 100);
    return { 
      score: similarity, 
      details: { 
        commonRedFlags: common,
        totalRedFlags: union,
        reason: `共同红线${common.length}/${union.length}，价值观一致度${similarity}%` 
      } 
    };
  },
  {
    displayName: '关键词红线',
    description: '检查双方红线是否一致，一致反而说明价值观契合',
    paramsSchema: {
      separator: { type: 'string', default: ',', label: '分隔符' }
    }
  }
